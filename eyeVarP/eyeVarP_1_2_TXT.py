...
#			elif ((this_line[j] == "FORMAT") and predictors ): # ok to stop checking for predictors 
				predictors=False #
				eyeVarP_conf.txt_end=j #
#				print "predictors = FALSE"
#
			if (predictors and this_line[j] !="Alt") : # true
				if any( s in this_line[j] for s in eyeVarP_conf.Population ) : # when filtering for QC value 
					eyeVarP_conf.pop_array.append([eyeVarP_conf.PF,this_line[j],eyeVarP_conf.MAFval]) 
				eyeVarP_conf.pred_array.append([eyeVarP_conf.PD,this_line[j],"N","","0","","1","","2"]) 
##
###########################################################################################################
#
###########################################################################################################
...
#	global txt_end 
#
	print ("eyeVarP_txt.setup_default_pred_values: ") #
##
...
#			if (this_line[0] !="Chr") : # skip header line
			if (this_line[0] !="#CHROM") : # skip header line
				try: #
					for j in range(eyeVarP_conf.txt_start,eyeVarP_conf.txt_end): # keep values - start with after Alt column and ends with FORMAT 
						pred_array_pos=j-eyeVarP_conf.txt_start     # current eyeVarP_conf.pred_array position for value 
#					print "eyeVarP_conf.pred_array_pos :",eyeVarP_conf.pred_array_pos     # current eyeVarP_conf.pred_array position for value 
#					print "this line :",this_line[j]     # current eyeVarP_conf.pred_array position for value 
						if ((this_line[j] != ".") and (this_line[j] != "-") and (this_line[j] != "-999")): 
#						if (eyeVarP_conf.is_number(this_line[j])): # numeric
							if (eyeVarP_conf.is_number(this_line[j]) and (eyeVarP_conf.pred_array[pred_array_pos][eyeVarP_conf.PD_type] != "A" )): # numeric
								this_numeric=True
								if ((eyeVarP_conf.pred_array[pred_array_pos][eyeVarP_conf.PD_low] == "") or (float(eyeVarP_conf.pred_array[pred_array_pos][eyeVarP_conf.PD_low]) > float(this_line[j]))): 
									eyeVarP_conf.pred_array[pred_array_pos][eyeVarP_conf.PD_low] = this_line[j] 
								if ((eyeVarP_conf.pred_array[pred_array_pos][eyeVarP_conf.PD_high] == "") or (float(eyeVarP_conf.pred_array[pred_array_pos][eyeVarP_conf.PD_high]) < float(this_line[j]))): 
									eyeVarP_conf.pred_array[pred_array_pos][eyeVarP_conf.PD_high] = this_line[j] 
							else : # if not a value that can be expressed as a floating point then is alpha
#							print "except", this_line[i] 
								this_numeric=False
								if ((eyeVarP_conf.pred_array[pred_array_pos][eyeVarP_conf.PD_low] == "") or (eyeVarP_conf.pred_array[pred_array_pos][eyeVarP_conf.PD_low] > this_line[j])): 
									eyeVarP_conf.pred_array[pred_array_pos][eyeVarP_conf.PD_low] = this_line[j] 
								if ((eyeVarP_conf.pred_array[pred_array_pos][eyeVarP_conf.PD_high] == "") or (eyeVarP_conf.pred_array[pred_array_pos][eyeVarP_conf.PD_high] < this_line[j])): 
									eyeVarP_conf.pred_array[pred_array_pos][eyeVarP_conf.PD_high] = this_line[j] 
								eyeVarP_conf.pred_array[pred_array_pos][eyeVarP_conf.PD_type] = "A" 
								aa=len(eyeVarP_conf.pred_array[pred_array_pos]) 
#							print aa #
...
#								print k, aa #
									while (k <= aa) :
										if (k == aa): # completed search and not found	
											eyeVarP_conf.pred_array[pred_array_pos].append(this_line[j]) # then add it
#										print "add- ",this_line[i+1],"/",this_line[i] 
										else : # keep searching
											if (eyeVarP_conf.pred_array[pred_array_pos][k] == this_line[j] ) : # current value already added 
												break #
...
#				print("pred_array index when error occurred :", j) #
					print("pred_array when error occurred :", eyeVarP_conf.pred_array[pred_array_pos]) #
					sys.exit(1) #
#
#	print pred_array #
	#
...
#				print ("checking values: ",eyeVarP_conf.sample_loc,eyeVarP_conf.FORMAT_loc,eyeVarP_conf.sample_coverage_loc[0]) #
				if ((eyeVarP_conf.sample_loc >=0) and (eyeVarP_conf.FORMAT_loc >=0) and (eyeVarP_conf.sample_coverage_loc[0] >=0)): #
#					print "OK" #
					eyeVarP_conf.header_ln = eyeVarP_conf.header_ln+tab+eyeVarP_conf.sample_ID # add sample ID to header line
#					print "header- ",eyeVarP_conf.header_ln #
#					outline1 = eyeVarP_conf.header_ln+nl #
					eyeVarP_conf.blank_variant_ln = eyeVarP_conf.blank_variant_ln+tab+"0" # this is a default variant line for samples that do not have this variant
#					print "blank_variant_ln- ",eyeVarP_conf.blank_variant_ln #
#					print sample_coverage, "/", sample_coverage_loc #
					...
					elif (os.stat(eyeVarP_conf.working_file1).st_size == 0): # there any no variants for this sample 
...
#						print "SAMPLE: ",eyeVarP_conf.sample_loc,"/",eyeVarP_conf.sample_ID #
					elif (content == "FORMAT") : # 
						eyeVarP_conf.FORMAT_loc=i #save FORMAT location
#						print "FORMAT_loc: ",FORMAT_loc #
				#	
				if ("#CHROM" not in eyeVarP_conf.header_ln): # is this first time
#					print ("setup header") #
					eyeVarP_conf.header_ln='\t'.join(src_line1[:eyeVarP_conf.FORMAT_loc+1])+tab+eyeVarP_conf.GENE_NAME # save heading up to INFO (using INFO_loc +1 to denote the stop field)
#					eyeVarP_conf.header_ln=eyeVarP_conf.header_ln+tab+eyeVarP_conf.GENE_NAME # 
#					eyeVarP_conf.blank_variant_ln='\t'.join(src_line1[:eyeVarP_conf.FORMAT_loc+1]) # setup dummy variant line
					eyeVarP_conf.blank_variant_ln=eyeVarP_conf.header_ln # save heading up to INFO (using INFO_loc +1 to denote the stop field)
#					print "header_ln : ",eyeVarP_conf.header_ln #
			else : # variants lines 
#				print ("var line : ",src_line1) #
				FORMAT1=re.split(':',src_line1[eyeVarP_conf.FORMAT_loc]) # split the variant line's FORMAT field
#				print ("FORMAT : ",src_line1[eyeVarP_conf.FORMAT_loc]) # split the variant line's FORMAT field
...
#						print (eyeVarP_conf.sample_coverage[j],"/",content) #
						if (content == eyeVarP_conf.sample_coverage[j]) : 	# look for the coverage FORMAT fields 
#							print (i,"/",j,"/",eyeVarP_conf.sample_coverage_loc)
							eyeVarP_conf.sample_coverage_loc[j]=i 			# save its location	
							break #
#				print (i,"/",j)
#				print ("coverage : ",eyeVarP_conf.sample_coverage_loc) #
...
#				print SAMPLE1 #
#				print eyeVarP_conf.sample_coverage_loc #
# have a coverage check on the sample 
#				if (((eyeVarP_conf.sample_coverage_loc[eyeVarP_conf.NR_val] == -1) or ((SAMPLE1[eyeVarP_conf.sample_coverage_loc[eyeVarP_conf.NR_val]] != ".") and (int(SAMPLE1[eyeVarP_conf.sample_coverage_loc[eyeVarP_conf.NR_val]]) >= eyeVarP_conf.Maxcoverage))) and #  NR
#					((eyeVarP_conf.sample_coverage_loc[eyeVarP_conf.DP_val] == -1) or ((SAMPLE1[eyeVarP_conf.sample_coverage_loc[eyeVarP_conf.DP_val]] != ".") and (int(SAMPLE1[eyeVarP_conf.sample_coverage_loc[eyeVarP_conf.DP_val]]) >= eyeVarP_conf.Maxcoverage)))) : # DP
#				if (SAMPLE1[eyeVarP_conf.sample_coverage_loc[eyeVarP_conf.GT_val]] != "./.") : # a valid genotype 
				if (SAMPLE1[eyeVarP_conf.sample_coverage_loc[eyeVarP_conf.GT_val]] != "./.") and (SAMPLE1[eyeVarP_conf.sample_coverage_loc[eyeVarP_conf.GT_val]] != "0/.") and (SAMPLE1[eyeVarP_conf.sample_coverage_loc[eyeVarP_conf.GT_val]] != "./0") and (SAMPLE1[eyeVarP_conf.sample_coverage_loc[eyeVarP_conf.GT_val]] != "0/0")  : # a valid genotype 
...
#						Sample_coverage=int(SAMPLE1[eyeVarP_conf.sample_coverage_loc[eyeVarP_conf.NR_val]])+int(SAMPLE1[eyeVarP_conf.sample_coverage_loc[eyeVarP_conf.NV_val]]) # save DP value
						...
						Alt_reads=int(SAMPLE1[eyeVarP_conf.sample_coverage_loc[eyeVarP_conf.NV_val]]) # save DP value
#						print ("NR+NV") #
...
#					if (SAMPLE1[eyeVarP_conf.sample_coverage_loc[eyeVarP_conf.NR_val]] == ".") : # no DP_val 
...
#
					...
					if ((Sample_coverage >= int(eyeVarP_conf.Maxcoverage)) and (int((Alt_reads/Sample_coverage)*100) >= int(eyeVarP_conf.Hete_Balance))) : # Pas QC for coverage and balance
...
##	COMMAND="sort -V -u -k1,5 "+eyeVarP_conf.working_file1+" > "+eyeVarP_conf.sort_file1 #  
	COMMAND="sort -V -u "+eyeVarP_conf.working_file1+" > "+eyeVarP_conf.sort_file1 #  
...
#
	...
	header1=re.split('\t|\n|\r',eyeVarP_conf.header_ln) # split into file location and sample id
#	print "header : ",eyeVarP_conf.header_ln #
#	print (src_line1) #
...
#			eyeVarP_conf.gene_ref=src_line1[gene] # now add in the gene ref 
#
...
#		elif (SAMPLE1[eyeVarP_conf.sample_coverage_loc[eyeVarP_conf.GT_val]] == "1/1") : # a homozygous alt genotype 
			...
			gtype="1" 
#
		outline='\t'.join(src_line1[:eyeVarP_conf.FORMAT_loc+1])+tab+eyeVarP_conf.gene_ref+tab+gtype+nl #
		wrkf1.write(outline) #
#	else : #
...
#			print ("content-",content,"/",i,"/",eyeVarP_conf.PF_array[j][1]) 				#
...
#						print (info_ln[i],"/",eyeVarP_conf.PF_array[j][2]) #
##				if ( float(info_ln[i]) > float(eyeVarP_conf.PF_array[j][2]) ) :	 # when number is < 0.0001 it is expressed as e-0x 
						if ( float(temp_val) > float(eyeVarP_conf.PF_array[j][2]) ) :	 # when number is < 0.0001 it is expressed as e-0x 
#						print ("do not want : ",info_ln[i],"/",eyeVarP_conf.PF_array[j][2]) #
...
#	print info_ln[1] #
	...
	for i, content in enumerate(header1): # return the value and index number of each item in the line array 
#		print "content-",content,"/",i,"/",eyeVarP_conf.PF_array[j][1] 				#
		if (content == eyeVarP_conf.GN_value ) : # the gene ref annotation we want? 
			eyeVarP_conf.gene_ref=info_ln[i] # save gene name 
			break #
#	
###########################################################################################################
#
...
#		print (eyeVarP_conf.PD_array[j][1]) # move to pred_array slot
		pdval=0 #
		for i, content in enumerate(header1): # return the value and index number of each item in the line array 
#			print "content-",content,"/",i				#
#			print content #
...
#							if ( INFO_details[i] == eyeVarP_conf.PD_array[j][a_value] ) :		# is this the variant type we are looking for   
...
#							while (a_value < len(eyeVarP_conf.PD_array[j])-1) : # loop thru the options for predictor scores 
#								if ( float(INFO_details[i]) < float(eyeVarP_conf.PD_array[j][a_value])) :	 # is this the variant type we are looking for   
#									val=val+int(eyeVarP_conf.PD_array[j][a_score])	 		# yes - return the score for the variant 
#									break 										# finish 
#								if ( a_value+2 >= len(eyeVarP_conf.PD_array[j])-1) : # end of this PD limits  
#								print ("checking last value") #
#									if ( float(INFO_details[i]) > float(eyeVarP_conf.PD_array[j][a_value])) :	 # check the last value, which is a greater then check   
#										val=val+int(eyeVarP_conf.PD_array[j][a_score])	 		# yes - return the score for the variant 
#								a_value+=2	 # bump to next value option   
#								a_score+=2	 #   
