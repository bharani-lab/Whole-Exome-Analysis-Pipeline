...
#                  - added code for a utility to convert VEP annotated VCF into standard format VCF which can be used by eyeVarP
# eyeVarP - version 1 - 26/08/2020
###########################################################################################################
#
import sys, re, glob, os, subprocess, time #
import numpy as np #
import eyeVarP_conf, eyeVarP_1_prioritise, eyeVarP_2_Gene, eyeVarP_3_sample_selection, eyeVarP_4_stats, eyeVarP_5_merge, eyeVarP_6_utility #
...
info_opt0_msg1=["#eyeVarP version 2 - 07/01/2021 ",
...
"#              format -  convertVEP -  a utility function to convert VEP annotated VCF into a standard format VCF which can be used by eyeVarP ",
"#inpt2=$3 # for tool ",
...
#
	if (supplied_args < 2 ):  # arg [0] is the python program
		...
		eyeVarP_conf.eyeVarP_option=sys.argv[1] #
#		print (eyeVarP_conf.eyeVarP_option) #
...
##
	print ("clean_up():") 
	for f in glob.glob(eyeVarP_conf.output_dir+"*_"+suffix+"*_tmp.txt") : # 
		os.remove(f) #
#
###########################################################################################################
#
...
#		print "no good" #
		return #
#
	if (eyeVarP_conf.eyeVarP_option=="priority"): #
#		print ("opt1") 
		eyeVarP_1_prioritise.main() #
	elif (eyeVarP_conf.eyeVarP_option=="genef"): #
#		print ("opt2") 
		eyeVarP_2_Gene.main() #
	elif (eyeVarP_conf.eyeVarP_option=="samplef"): #
#		print ("opt3") 
		eyeVarP_3_sample_selection.main() #
		if (eyeVarP_conf.inh_model in ["CH"]) : # for CH model we need to only return variants that are for genes that have variants in both parents
#			print ("CH_inh_model") #
			eyeVarP_conf.input_file=eyeVarP_conf.temp_output_file #
			eyeVarP_conf.gene_list=eyeVarP_conf.sort_file3 #
			eyeVarP_2_Gene.filter_the_variants() #
	elif (eyeVarP_conf.eyeVarP_option=="stats"): #
#		print ("opt4") 
		eyeVarP_4_stats.main() #
	elif (eyeVarP_conf.eyeVarP_option=="merge"): #
#		print ("opt5") 
		eyeVarP_5_merge.main() #
	elif (eyeVarP_conf.eyeVarP_option=="utility"): #
#		print ("opt6") 
		eyeVarP_6_utility.main() #
	elif (eyeVarP_conf.eyeVarP_option=="help"): #
		for j in range(len(info_opt0_msg1)): #
			print (info_opt0_msg1[j]) #
	else :
		print ("eyeVarP : Invalid tool option specified - please check input command ")
		for j in range(len(info_opt0_msg1)): #
			print (info_opt0_msg1[j]) #
#		
...
#
# module load python3
# Entry : python3 eyeVarP_main.py <option> <input file1> <input file2> <input file3>
#
###########################################################################################################
